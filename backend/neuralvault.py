...
def parse_query(query: str) -> dict:
    ...
    prompt = (
        ...
        'Type hints — ONLY set type_filter when the user explicitly names a file type:\n'
        '  "photo/screenshot/picture" → ["image"]  (NOT "slides" or "diagram")\n'
        '  "video/movie/clip/recording" → ["video"]\n'
        '  "audio/music/song/podcast" → ["audio"]\n'
        '  "pdf/document/doc/report" → ["pdf","other"]\n'
        '  "code/script/program" → ["code"]\n'
        'When in doubt, set type_filter to null and let semantic search find it.\n\n'
        'Respond with JSON only, no markdown:\n'
        '{"semantic_query": "...", "type_filter": ["image"] or null}\n\n'
        'Examples:\n'
        '"vacation photos" → {"semantic_query": "vacation", "type_filter": ["image"]}\n'
        '"presentation slides" → {"semantic_query": "presentation slides", "type_filter": null}\n'
        '"lecture notes" → {"semantic_query": "lecture notes", "type_filter": null}\n'
        '"my resume" → {"semantic_query": "resume", "type_filter": null}\n'
        '"sorting algorithm code" → {"semantic_query": "sorting algorithm", "type_filter": ["code"]}\n'
        '"a photo of the beach" → {"semantic_query": "beach", "type_filter": ["image"]}\n'
